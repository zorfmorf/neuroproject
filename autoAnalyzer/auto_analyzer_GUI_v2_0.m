...
function auto_analyzer_GUI_v2_0()
...
hFig = figure('Units','normalized','Position',[1.0 0.05 .49 .85],'MenuBar','None');

% ------------Set menu and toolbar-----------------------------------------
menuFile = uimenu(hFig,'Label','File');
uimenu(menuFile,'Label','Load .mat tracks','Callback',@LoadMatTracksCB);
uimenu(menuFile,'Label','Batch Processing','Callback',@BatchProcessingCB);
handles.uiMenu.Video = uimenu(menuFile,'Label','Create Movie','Callback',@MakeVideoCB, 'Enable','off');

menuAnalysis = uimenu(hFig,'Label','Analysis');
handles.uiMenu.TrackLifes = uimenu(menuAnalysis,'Label','Track lifes','Callback',@AnalysisModeCB,'Checked','on');
handles.uiMenu.Hoechst = uimenu(menuAnalysis,'Label','Hoechst regions','Callback',@AnalysisModeCB);
handles.uiMenu.Colocalization = uimenu(menuAnalysis,'Label','Colocalization','Callback',@AnalysisModeCB);
handles.uiMenu.Diffusion = uimenu(menuAnalysis,'Label','Diffusion','Callback',@AnalysisModeCB);

menuSettings = uimenu(hFig,'Label','Settings');
uimenu(menuSettings,'Label','Acquisition Parameters','Callback',@UpdateSettingsCB);% TODO change settings
uimenu(menuSettings,'Label','Fitting Parameters','Callback',@UpdateSettingsCB);
uimenu(menuSettings,'Label','Filter Preferences','Callback',@UpdateSettingsCB);
uimenu(menuSettings,'Label','Tracking Parameters','Callback',@UpdateSettingsCB);
...
handles.uiControls.ckboxBandpass = uicontrol(uiArgs.Chkbox{:},'Position',[.89 .85 .1 .03],'String','Bandpass Filter','Callback',@UpdateSettingsCB,'Enable','on');
...
handles.uiControls.textFrames1 = uicontrol(uiArgs.Text{:},'Position',[0.78 0.70 0.08 0.035],'String','Analyse frames from');
handles.uiControls.textFrames2 = uicontrol(uiArgs.Text{:},'Position',[0.92 0.685 0.08 0.035],'String','to');
...
uicontrol(uiArgs.Text{:},'Position',[.32 .16 .3 .03],'String','Tracklength range');
uicontrol(uiArgs.Text{:},'Position',[.36 .135 .1 .03],'String','to');
...
%Executed by mouse wheel for scrolling through frames
    ...
    function BatchProcessingCB(~,~)
        ...
        d = dialog('Position',[400 500 300 150],'Name','Select One','KeyPressFcn',@keyPressedCB);
        bg = uibuttongroup('Parent',d,'Visible','off','Position',[0 3/10 1 2/3]);
        ...
        uicontrol(bg,'Style','radiobutton',...
            ...
            'Position',[10 55 200 30],...
            'HandleVisibility','off');
        
        textBox = uicontrol(bg,'Style','edit',...
            'String','4',...
            'Position',[200 40 60 30],...
            'HandleVisibility','off');
        ...
        for n = 1:numberfiles
            if numberfiles ~= 1
                filename = char(filename(n)); %Current filename
            else
                filename = char(filename);
            end
            stack_1 = load_images(filename); %Load current stack
        ...
        for n = 1:numberfiles
            ...
            stack_1 = load_images(filename);
            
            %% ------Filter Stack, adjust SNR and InT for finding and fitting spots------
            useBandpass = 0;
            [denoised,oribgrnd,meQ,sdQ] = filter_stack(stack_1,ROIs{n},useBandpass); %Filter stack in ROI
            
            params = handles.params;
            if strcmp(choiseInTSNR,'Intensity threshold (InT)')
                params.intensityThreshold = InTSNR;
                params.SNR = round((InTSNR - meQ) / sdQ,1);
            else
                params.SNR = InTSNR;
            ...
            %% ---Save .mat file--------------------------------
            save([videoName,'_GUI'], 'videoName', 'denoised',...
    ...
    function LoadMatTracksCB(~,~)
        ...
        load(filename,'ROIlist','ROElist','ROI','spotsFiltered','spotsAllInROI',...
            'tracksAll','tracksFiltered','denoised','oribgrnd',...
        ...
        handles.uiControls.editTrackMax.String = size(tracksFiltered,2);
    ...
    function LoadStackCB(~,~)
        ...
        [stack_1, ~] = load_images(filename); %Second argument contains nikon tif-tag info
        handles.data.stack_1.imageData.original = stack_1;
        ...
        handles.uiControls.popSubstack.Enable = 'off'; %Disable substack selection menu
        set(gcf, 'WindowScrollWheelFcn', {@wheel,handles});%Set mouse wheel handle for scrolling through frames
        
        guidata(hFig,handles); %Save handles structure (is needed in Reset function)
        Reset('LoadStack');  % Reset all Spots, Tracks etc. and reinitialize GUI        
        
        %% -----Set Subfolder and filename for saving  
        handles = guidata(gcf);
               
        videoName  = filename(1:end-4); %filename without .tif extension
        uScore = strfind(filename, '_'); %Positions of underscores in filename
        try %Try if name follows convention (jjmmdd_name_intensity_exposureTime_timelapseCondition_videoNumber)?
            date = filename(3:7); %mmdd_
            name = filename(8:(8+uScore(1,2)-uScore(1,1)-1)); %Name_
            expTime = filename(strfind(filename, '_e')+2:strfind(filename, '_e')+4); %Exposure
            timelapse = filename(strfind(filename, '_t')+2:strfind(filename, '_t')+4); %Timelapse condition
            number = filename(strfind(filename, '_n')+2:strfind(filename, '_n')+4); %Video number
            subFolderName = strcat(name, date, expTime,timelapse); %Subfolder
            saveName = strcat(subFolderName, '_', number); %Name for saving
        catch
            disp('Filename does not follow convention, whole filename will be used for saving.')
            subFolderName = videoName;
            saveName = videoName;
        end
        
        subPathName = fullfile(pathName, subFolderName);
        
        %% -----Load tracks if .mat file exists
        
        if exist(fullfile(pathName, subFolderName,[videoName,'.mat']), 'file')
            cd(char(subPathName));
                    load([videoName,'.mat'],'ROIlist','ROElist','ROI','spotsFiltered','spotsAllInROI',...
            'tracksAll','tracksFiltered','denoised','oribgrnd',...
            'MaxJump','MinLength','DarkFrames','params')
        ...
        if exist('params.SNR', 'var')
            handles.uiControls.editSNR.String = params.SNR;
        else
            disp('SNR has not been loaded/found')
        end
        handles.uiControls.editInT.String = params.intensityThreshold;
        handles.uiControls.editMaxJump.String = MaxJump;
        handles.uiControls.editMinLength.String = MinLength;
        ...
        elseif exist([videoName,'_GUI', '.mat'], 'file')
            
            load([videoName,'_GUI', '.mat'], 'denoised', 'oribgrnd', 'spots', 'ROI','ROIlist',...
                'meQ','sdQ','params');
        ...
        %% -----------Update GUI and store handles-----------
        handles.data.names = struct( ...
            'videoName',       filename(1:end-4), ...     %former matName
            'pathName',        pathName,...       %
            'saveName',        saveName, ...     %
            'subFolderName',   subFolderName, ...   %
            'subPathName',     subPathName);     %Subfolder path
                        
        curFrame = round((get(handles.uiControls.sliderFrame,'Value')));
        imshow(stack_1{curFrame},'DisplayRange',[min(min(stack_1{1})) max(max(stack_1{1}))],'Parent',handles.axes1);
        handles.uiControls.textStackName.String = strcat('Stack name:',{' '}, filename);
    ...
    function DivideIntoSubstacksCB(~,~)
        ...
        if saveTif
            ...
            for i = 1:2 %Iterate through substacks
                ...
                filename = strcat('Substacks/',handles.data.names.videoName,'_Substack_',num2str(i),'.tif');
                imwrite(I,filename,'Compression','none','WriteMode','overwrite')
                for j = 2:stackCounter(i) %iterate through frames of substack
                    I=uint16(substack{i,j});
                    imwrite(I,filename,'Compression','none','WriteMode','append')
                end
            end
        ...
        handles.stack_2.seq = seq; %Store sequencing numbers in handles structure
        
        %% ------Store the substsacks in callbacks
        handles.data.stack_1.imageData.original = substack(1,1:stackCounter(1));
        handles.data.stack_2.imageData.original = substack(2,1:stackCounter(2));
    ...
    function Reset(src,~)
        %Called by Filter,Reset,LoadStack and DivideIntoSubstacksCB Buttons
        handles = guidata(gcf);
        ...
        handles.data.stack_1.spotsFiltered = cell(1,numFrames);
        handles.data.stack_1.spotsAll = cell(1,numFrames);
        handles.data.stack_2.spotsFiltered = cell(1,numFrames);
        handles.data.stack_2.spotsAll = cell(1,numFrames);
        handles.data.stack_1.spotParams.intensityThreshold = 0;
        handles.data.stack_2.spotParams.intensityThreshold = 0;
        
        %---------Delete ROI, ROE, tracks and boundaries
        if ~strcmp(src,'Filter')
            handles.data.regions.ROIlist = [0,0];
            handles.data.regions.ROElist = {};
            handles.data.regions.ROI = ones(size(stack{1})); %Initialize ROI to whole image
            handles.data.stack_1.tracksAll = {};
            handles.data.stack_1.tracksFiltered = {};
            handles.data.stack_2.tracksAll = {};
            handles.data.stack_2.tracksFiltered = {};
            handles.data.hoechst.boundaries1 = cell(1,numFrames);
            handles.data.hoechst.boundaries2 = cell(1,numFrames);
            handles.data.colocalizedTracks = {};
        end
        %--------------------------------------
        SetGUIState('Reset')
        guidata(hFig,handles);
    ...
    function FilterCB(src,~)
        ...
        [denoised,oribgrnd,handles.meQ,handles.sdQ] = filter_stack(stack_1,handles.ROI,handles.uiControls.ckboxBandpass.Value); %Filter stack in ROI
        ...
        if  strcmp(handles.uiControls.popSubstack.Enable,'on')
            ...
            [Denoised_2,Oribgrnd_2,handles.meQ_2,handles.sdQ_2] = filter_stack(Stack_2,handles.ROI,handles.uiControls.ckboxBandpass.Value);
...
%Executed when SNR or InT threshold is changed
    ...
    function SaveTxtAndMatCB(~,~) %TODO aufräumen!!   

        subPathName = handles.names.subPathName;
        saveName = handles.names.saveName;
        pathName = handles.names.pathName;
        videoName = handles.names.videoName;        
        txtTL = strcat(saveName, '_tl.txt'); %.txt name for tracklifes
        ...
        meQ = handles.meQ;%#ok<NASGU>
        sdQ = handles.sdQ;%#ok<NASGU>
        
        %--------save files-------------
        
        if ~exist(subPathName, 'dir')    % Folder does not exist so create it.
            mkdir(subPathName)
        end
        
        cd(char(subPathName));
        
        if strcmp(handles.uiMenu.TrackLifes.Checked,'on')
            ...
            txtAndy = strcat(saveName, '_myoEval.csv'); %.txt name for Spots Intensities
            tracksFiltered = handles.tracksFiltered;
            ...
            save(videoName, 'names')

            
%             save(videoName, 'videoName', 'stack_1', 'denoised',...
%                 'oribgrnd', 'params','meQ','sdQ', 'spotsAllInROI','ROI','ROIlist','ROElist',...
%                 'spotsFiltered', 'tracksAll','tracksFiltered',...
%                 'MaxJump','MinLength','DarkFrames')
            
        elseif strcmp(handles.uiMenu.Diffusion.Checked,'on')
            ...
            txtTL = strcat(saveName, '_diff.txt');
        ...
        elseif strcmp(handles.uiMenu.Hoechst.Checked,'on')
            ...
            Th1 = handles.Th1;%#ok<NASGU> %Area threshold 1
            Th2 = handles.Th2;%#ok<NASGU> %Area threshold 2
            ...
            txtTlArea1 = strcat(saveName, '_tl_areaGreen.txt');
            txtTlArea2 = strcat(saveName, '_tl_areaBlue.txt');
            txtTlArea3 = strcat(saveName, '_tl_areaWhite.txt');
            ...
            resultsName = strcat(videoName, '_spacially_resolved_analysis.txt');
            ...
            fclose(fid);
            save(videoName, 'videoName', 'stack_1', 'denoised', 'oribgrnd',...
                'stack_2', 'denoised_2', 'oribgrnd_2', 'params_2', ...
                'meQ', 'sdQ',...
                'spotsAllInROI_2', 'spotsFiltered_2', 'tracksAll_2',...
                'tracksFiltered_2', 'ROI','ROIlist','ROElist',...
                'MaxJump','MinLength','DarkFrames','seq', 'areas', 'boundaries1',...
                 'boundaries2','tracksInArea1', 'tracksInArea2',...
                'tracksInArea3','Th1','Th2'); %Th1&Th2 erst in v1_2_3_1 hinzugefügt!
        end
    ...
    function MakeVideoCB(~,~)
        stack = getappdata(hFig,'CurrentStack');
        [filename, pathName] = uiputfile({'*.avi'},'Save Video',handles.videoName);
        cd(char(pathName));
        ...
        if ~isequal(filename,0)
            ...
            for curFrame = 1:length(stack)
                ...
                %% ----------Show Image----------
                xLim = handles.axes1.XLim; %Get zoom factor
    ...
    function HoechstRegionsCB(~,~)
        ...
        sliderFrameTh1 = uicontrol('Units','normalized','Enable','on','Style','slider','Position',[.05 .7 .9 .1],'Min',min(Q),'Max',max(Q),'Value',min(Q),'SliderStep',[1/20 1/10]);
        sliderFrameTh2 = uicontrol('Units','normalized','Enable','on','Style','slider','Position',[.05 .4 .9 .1],'Min',min(Q),'Max',max(Q),'Value',min(Q),'SliderStep',[1/20 1/10]);
        ...
        function SliderThListenerCB
            ...
            Th1 = sliderFrameTh1.Value; %Get threshold values from sliders
            Th2 = sliderFrameTh2.Value;
    ...
    function AnalysisModeCB(src,~)
        handles = guidata(gcf);
        handlesArray = [handles.uiMenu.TrackLifes,handles.uiMenu.Hoechst,...
            handles.uiMenu.Colocalization, handles.uiMenu.Diffusion];
        ...
        handlesArray = [handles.uiControls.btnHoechstRegions, handles.uiControls.btnHoechstTrackAssign, handles.uiControls.btnCreateHistogram,...
            ...
            handles.uiControls.editColocalize,handles.uiControls.textColocalize,handles.uiControls.editDarkFrames,...
            handles.uiControls.textDarkFrames];
        set(handlesArray, 'Visible', 'off');
        
        if strcmp(src.Label,'Track lifes')
        ...
        elseif strcmp(src.Label,'Hoechst regions')
            handlesArray = [handles.uiControls.btnHoechstRegions, handles.uiControls.btnHoechstTrackAssign, handles.uiControls.btnCreateHistogram,...
                handles.uiControls.btnSave, handles.uiControls.btnFindTracks, handles.uiControls.ckboxPlotPixels,handles.uiControls.editDarkFrames,...
                handles.uiControls.textDarkFrames];
            set(handlesArray, 'Visible', 'on');
            
        elseif strcmp(src.Label,'Colocalization')
            handlesArray = [handles.uiControls.btnCreateHistogram,handles.uiControls.btnFindTracks,...
                ...
                handles.uiControls.textColocalize,handles.uiControls.editDarkFrames,...
                handles.uiControls.textDarkFrames];
            set(handlesArray, 'Visible', 'on');
            
        elseif strcmp(src.Label,'Diffusion')
...
%Executed when user changes settings in the lower part of the GUI (Original/Filtered/Substack/Slider/Average)
    ...
    function UpdatePlot(stackName)
        ...
        if handles.uiControls.popOriFil.Value == 2
    ...
    function SetGUIState(state)
        handlesLoadStackEnable = [handles.uiMenu.TrackLifes,handles.uiMenu.Hoechst,...
            handles.uiMenu.Colocalization, handles.uiMenu.Diffusion,...
            ...
            handles.uiControls.sliderFrame,handles.uiMenu.Video,handles.uiControls.btnSubstacks,...
        ...
        switch state
            ...
            case 'FindTracks' %Find Tracks or load .mat tracks
                ...
                if strcmp(handles.uiMenu.Hoechst.Checked, 'off') %In Hoechst analysis, regions have to be drawn before saving
